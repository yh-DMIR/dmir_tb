...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, x_mark):
        x = torch.cat([x, x_mark], dim=-1).transpose(-1, -2)#x.shape = [B,D+4,L]
        x = self.proj(x.reshape(*x.shape[:-1], self.num_p, self.patch))#x.shape = [B,D+4,num_p,patch]->[B,D+4,num_p,d_model] d_model是embedding特征的维度
        return x
