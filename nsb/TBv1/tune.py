...
def objective(trial):
    ...
    parser.add_argument('--beita', type=float,default=0, help='adjust mae')
    ...
    # args.beita = trial.suggest_float('beita', 1e-5,0.1, log=True)
    # 正则化
    # args.dropout = trial.suggest_float('dropout', 0.05, 0.3, step=0.05)
    # args.attn_dropout = trial.suggest_float('attn_dropout', 0.05, 0.2, step=0.05)
    ...
    # 层数
    # args.ia_layers = trial.suggest_int('ia_layers', 1, 3) # SOTA 中为 1 或 3
    # args.ca_layers = trial.suggest_int('ca_layers', 0, 3) # SOTA 中为 0, 1, 2, 3
    # pd_layers 在 SOTA 中恒为 1，故不搜索以简化空间
    # args.pd_layers = 1

    # # 学习率调度器
    # args.lradj = trial.suggest_categorical('lradj', ['type1', 'TST'])
    # if args.lradj == 'TST':
    #     args.pct_start = trial.suggest_float('pct_start', 0.1, 0.4)
    #
    # # 模型结构
    # args.d_model = trial.suggest_categorical('d_model', [64, 128, 256, 512])  # 覆盖了 SOTA 中的 64, 128, 512
    #
    # # 确保 n_heads 能整除 d_model
...
if __name__ == '__main__':
    ...
    filename = f"{args.data_path}_seqlen_{args.seq_len}_predlen_{args.pred_len}_results.txt"
    file_path = os.path.join(output_dir, filename)

    with open(file_path, 'w') as f:
        f.write(f"--- Optuna Results for Dataset: {args.data_path}, Seq_len: {args.seq_len}, Pred Len: {args.pred_len} ---\n\n")
        f.write(f"Total trials completed: {len(study.trials)}\n\n")
        f.write("--- Best Trial ---\n")
        f.write(f"mae+mse (object): {best_trial.value:.7f}\n\n")
        ...
        f.write("Params (Best Hyperparameters):\n")
        for key, value in best_trial.params.items():
            f.write(f"  - {key}: {value}\n")
