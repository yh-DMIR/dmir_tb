...
class Exp_Long_Term_Forecast(Exp_Basic):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.output_attention:
                    outputy,outputx = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                else:
                    outputy,outputx = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)

                f_dim = -1 if self.args.features == 'MS' else 0
                outputy = outputy[:, -self.args.pred_len:, f_dim:]
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)

                pred = outputy.detach().cpu()
    ...
    def train(self, setting):
        ...
        best_val_loss = float("inf") # 初始化一个变量来追踪最好的验证损失
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                f_dim = -1 if self.args.features == 'MS' else 0
                outputy = outputy[:, -self.args.pred_len:, f_dim:]
                outputx = outputx[:, -self.args.seq_len:, f_dim:]
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                batch_x = batch_x[:, -self.args.seq_len:, f_dim:].to(self.device)
                beita = self.args.beita
                loss = self.time_freq_mae(batch_y, outputy) + beita * self.time_freq_mae(batch_x, outputx)
            ...
            # --- 新增代码开始 ---
            # 每当 early_stopping 更新时，我们也更新 best_val_loss
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # fc1 - channel_decoder
                if self.args.output_attention:
                    outputs,outputx = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]

                else:
                    outputs,outputx = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
